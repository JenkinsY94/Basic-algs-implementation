// use virtual nodes to improve runtime
// but have backwash problem!
...
public class Percolation {
    ...
    public void open(int i, int j) {
        ...
        if (i == 1) {
            uf.union((i-1)*size+j, 0);
        }
        // union the bottom row opened nodes with (size*size+1).
        if (i == size) {
            uf.union((i-1)*size+j, size*size+1);
        }
        if (i-1 > 0 && grid[i-2][j-1]) {
            uf.union((i-1)*size+j, (i-2)*size+j);
        }
    ...
    public boolean percolates() {
        if (uf.connected(0, size*size+1)) return true;
        return false;
    }
